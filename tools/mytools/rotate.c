...
int main(int argc, char **argv) {
    float from[3] = {1.0/sqrt(3.0), 1.0/sqrt(3.0), 1.0/sqrt(3.0)};
    float to[3] = {0.0, 0.0, 1.0};
    float mtx[3][3];
    int i=0;

    printf("Creating matrix from y to z\n");
    fromToRotation(from, to, mtx);

    printf("Rotating from to match to:\n");
    for (i=0; i<3; i++) {
        printf("%f %f %f\n", mtx[i][0], mtx[i][1], mtx[i][2]);
    }

    to[0] = from[0] * mtx[0][0] + from[1] * mtx[0][1] + from[2] * mtx[0][2];
    to[1] = from[0] * mtx[1][0] + from[1] * mtx[1][1] + from[2] * mtx[1][2];
    to[2] = from[0] * mtx[2][0] + from[1] * mtx[2][1] + from[2] * mtx[2][2];
