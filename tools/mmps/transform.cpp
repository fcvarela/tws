...
void Transform::SetData(const Image& inImage, Image& outImage,
			...
			uint64_t outIndex)	     
{
  uint64_t iw = inImage.Width();
  uint64_t ih = inImage.Height();
  ...
  uint64_t sx = unsigned(floor(hw + lambda * sw));
  // Clamp in case of rounding errors
  if (sx >= iw) sx = 0;

  // Use unsigned so we don't have to test for negative indices

  uint64_t sy = unsigned(floor(hh - phi * sh));
  if (sy >= ih) sy = ih-1;

  Rgb rgb;
  uint64_t inIndex = sy * iw + sx;
...
void Transform::TransformImage(const Image& inImage, Image& outImage,
			       const TransformParams& params)
{
  uint64_t ow = outImage.Width();
  uint64_t oh = outImage.Height();
  double orgx = 0.5 * ow;
  double orgy = 0.5 * oh;
  // Scale so that half width is 1
  double scaleFactor = BasicScale(ow,oh) / params.scale;
  uint64_t ox, oy; uint64_t outIndex = 0;
			   ...
			   uint64_t outIndex)	     
{
  (void(ox));
  (void(oy));
  int64_t iw = inImage.Width();
  int64_t ih = inImage.Height();
  double orgx = 0.5 * iw;
  double orgy = 0.5 * ih;
			  ...
			  double& phi, double& lambda) const
{
  ...
  if (qm >=  0) {
    ...
    if (a == 1.0 && b == 1.0 && c == 1.0) {
      ...
      // This is a pouint64_t on the ellipsoid, so convert to lat long
