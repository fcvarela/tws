...
class Transform {
 public:
  ...
  virtual void AdjustSize (uint64_t& w, uint64_t& h, TransformParams& params) { 
    (void(w)); (void(h)); (void(params)); 
  };
  virtual double BasicScale(uint64_t width, uint64_t height) const = 0;
...
public:
  ...
  bool ProjectInv (const TransformParams& params,
		   double phi, double lambda, double& x, double& y) const;
  double BasicScale(uint64_t width, uint64_t height) const { (void(width)); return 2.0 / height; };
private:
...
public:
  ...
  bool ProjectInv(const TransformParams& params,
		  double phi, double lambda, double& x, double& y) const;
  double BasicScale(uint64_t width, uint64_t height) const { (void(height)); return 2.0 * pi / width; };
...
class Hammer : public SimpleTransform
{
 public:
  void Init(const TransformParams& params);
  double BasicScale(uint64_t width, uint64_t height) const { (void(height)); return 2.0 / width; };
