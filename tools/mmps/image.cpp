...
void Image::Copy (const Image& image) {
  if (data == NULL) {
    ...
    for (size_t y = 0; y < height && y < image.height; y++) {
      for (size_t x = 0; x < width && x < image.width; x++) {
	Rgb rgb;
...
void Image::Read (const char* filename, bool writeable)
{
  ...
  uint32_t w; uint32_t h; uint32_t ncolors; size_t datasize;
  ...
  datasize = (fileend-fileptr);
  fprintf(stderr, "Width: %d Height: %d, Data size: %ld File calc size: %ld\n", w, h, (long)datasize, (long)(fileend-fileptr));
