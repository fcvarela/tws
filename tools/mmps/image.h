...
class Image {
 public:
  ...
  inline void CheckSetRgb(size_t i, size_t j, const Rgb& rgb) {
    if (i >= 0 && i < width && j >= 0 && j < height) {
      SetRgb(j * width + i, rgb);
    }
  }
  inline void CheckGetRgb(size_t i, size_t j, Rgb& rgb) {
    if (i >= 0 && i < width && j >= 0 && j < height) {
  ...
  inline void SetRgb(size_t i, size_t j, const Rgb& rgb) {
    SetRgb(j * width + i, rgb);
  }
  inline void GetRgb(uint64_t i, uint64_t j, Rgb& rgb) const {
    GetRgb(j * width + i, rgb);
  }
  ...
  inline void GetRgb(uint64_t i, Rgb& rgb) const {
#if defined UNALIGNED
    Rgb* p = reinterpret_cast<Rgb*>(data+i+i+i);
    rgb = *p;
#else
